...
import uk.tfl.gov.drivermanager.ManageDriver;
...
/*
    ...
    // method to return List<Double> (after removing $ symbol) of price in Ascending order
    public void verifyIfProductsAreSortedByPriceHighToLow(WebElement element1, WebElement element2) {
        List<WebElement> afterSortingPriceListOfWebElement = driver.findElements(lcProductsPriceWebElementsOfAllListed); // list of all price WebElements
        List<Double> actualListInDouble = new ArrayList<>(); // creating empty ArrayList for double values./ after removing currency symbol
        for (WebElement productsTextElements : afterSortingPriceListOfWebElement) { // for each loop to run through all webelements ony by one
            //  productsTextElements.getText().replace("$","");// replace $ with ""
            //  Double.valueOf(productsTextElements.getText().replace("$","")); // convert into Double type.
            actualListInDouble.add(Double.valueOf(productsTextElements.getText().replace("$", ""))); // once converted into double --> it will store in Double list Array List
        }
        System.out.println("actual Price list of text only before sorting=" + actualListInDouble);
